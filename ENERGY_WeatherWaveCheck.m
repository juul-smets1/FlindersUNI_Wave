% Load organized data
organized_data = readtable('organized_data_aligned_D_E_Weather.csv');
...
direction_bins = [-Inf, 210:3:270, Inf]; %This can be [-Inf, 210:3:270, Inf]; or [0:30:360];
...
for i = 1:length(locations)
    if strcmp(locations{i}, 'Bri') || strcmp(locations{i}, 'Sem') % Check if the location is Brighton or Semaphore
        ...
        wave_period = organized_data.([location '_Tp_s']);  

        % Remove NaN values
        valid_indices = ~isnan(wave_directions) & ~isnan(wave_period);
        wave_directions = wave_directions(valid_indices);
        wave_period = wave_period(valid_indices);

        % Bin wave periods and directions
        period_index = discretize(wave_period, period_bins_custom{i});
        ...
        h = heatmap(direction_bins(1:end-1), period_bins_custom{i}(1:end-1), bin_counts, ...
            'XLabel', 'Wave Direction (°N)', 'YLabel', 'Wave Period (s)', ...
            'Colormap', cmap);
        title(h, sprintf('Wave Period Distribution at %s during the weather events(Total Count: %d)', full_name, total_count));
